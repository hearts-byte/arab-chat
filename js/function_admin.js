...
saveAdminDisplay = function(){
	$.post('system/action/system_save.php', { 
		...
		set_log_mode: $('#set_log_mode').val(),
		set_left_mode: $('#set_left_mode').val(),
...
saveAdminUserPermission = function(){
	$.post('system/action/system_save.php', { 
		...
		set_allow_pmusic: $('#set_allow_pmusic').val(),
...
saveAdminStaffPermission = function(){
	$.post('system/action/system_save.php', { 
		...
		set_can_pmusic: $('#set_can_pmusic').val(),
...
saveAdminCall = function(){
	$.post('system/action/system_save.php', { 
		...
		set_can_agcall: $('#set_can_agcall').val(),
		set_can_vgcall: $('#set_can_vgcall').val(),
	...
	if(roomAdmin == 0){
		...
		$.post('system/action/action_rooms.php', { 
			...
			admin_set_ricon: $('#set_room_icon').attr('data'),
				...
				if(response.code == 1){
					$('#room_listing').prepend(response.data);
					hideModal();
				}
				roomAdmin = 0;
		}, 'json');
		...
		$.post('system/action/system_console.php', {
			reload_console: lastConsole,
				...
				if(response.code == 1){
					...
					$('#console_results').prepend(response.data);
				}
		}, 'json');
	}
}
	...
	boomDelay(function() {
		...
		$.post('system/action/system_console.php', {
			search_console: $('#search_system_console').val(),
				...
				if(response.code == 1){
					$('#console_spinner').hide();
					$('#console_results').html(response.data);
				}
		}, 'json');
	}, 1000);
}
...
addGift = function(){
	...
	if(!validUpload('add_gift', fileMax)){
		return false;
	}
	else {
		if(waitGift == 0){
...
adminCancelCall = function(id){
	$.post('system/action/action_scall.php', {
			...
			if(response.code == 1){
				$('#admincall'+id).replaceWith(response.data);
				hideModal();
			}
	}, 'json');	
}
reloadAdminCall = function(){
	$.post('system/action/action_scall.php', {
			...
			if(response.code == 1){
				$('#admin_calls').html(response.data);
			}
	}, 'json');	
}
reloadAdminGroupCall = function(){
	$.post('system/action/action_scall.php', {
			...
			if(response.code == 1){
				$('#admin_group_calls').html(response.data);
			}
	}, 'json');	
}
adminLoad = function(){
...
$(document).ready(function(){
		...
		if(validSearch >= 1){
			$.post('system/action/action_search.php', {
				search_member: $('#member_to_find').val(),
					...
					$('#member_list').html(response.data);
			}, 'json');
		}
	});
		...
		if(actionCritera == 1000){
			...
			$.post('system/action/action_search.php', {
				search_action: $(this).val(),
					...
					if(response.code == 1){
						$('#action_listing').html(response.data);
					}
			}, 'json');
		}
	});
	...
	$(document).on('click', '.update_system', function(){
		if(waitUpdate == 1){
			...
			$(this).hide();
			$(this).prev('.work_button').show();
			$.post('system/action/system_update.php', {
