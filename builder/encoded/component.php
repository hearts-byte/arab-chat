...
function processInstall()
{
    ...
    // التحقق من الاتصال والإعدادات
    $q = "SELECT @@sql_mode AS config";
    $result = $mysqli->query($q);

    if ($result === false) {
        return boomCode(0, ['error' => 'Database query failed: ' . $mysqli->error]);
    }
    ...
    // إدخال بيانات الإعدادات في جدول DB
    $mysqli->query("INSERT INTO `boom_setting` (`id`, `title`, `domain`, `language`, `timezone`) VALUES (1, '$title', '$domain', '$language', 'America/Toronto')");

    // إنشاء حساب الأدمن (رتبة 100)
    $mysqli->query("INSERT INTO `boom_users` (`user_name`, `user_password`, `user_email`, `user_ip`, `user_join`, `user_rank`, `user_theme`, `user_language`) VALUES ('$username', '$password', '$email', '" . getIp() . "', '$time', 100, 'Dark', '$language')");

    $admin_id = $mysqli->insert_id;

    if ($admin_id) {
        // إنشاء السجلات الإضافية للمسؤول
        $mysqli->query("INSERT INTO `boom_users_data` (`uid`) VALUES ('$admin_id')");
        $mysqli->query("INSERT INTO `boom_exp` (`uid`) VALUES ('$admin_id')");

        // إنشاء الغرفة الافتراضية للدردشة
        $mysqli->query("INSERT INTO `boom_rooms` (`room_name`, `topic`, `description`, `room_icon`, `room_system`, `room_creator`) VALUES ('Lobby', 'Welcome to our chatroom', 'Main chat room', 'default_room.png', 1, '$admin_id')");

        return boomCode(1, ["success" => "Installation completed successfully!"]);
    } else {
        return boomCode(0, ["error" => "Failed to create admin account. Please try again."]);
    }
}
?>
