...
<?php if(leftMode(2)){ ?>
<link rel="stylesheet" type="text/css" href="css/left_menu.css<?php echo $bbfv; ?>" />
<?php } ?>
<?php if(!boomLogged()){ ?>
...
document.addEventListener('DOMContentLoaded', () => {
  if ('serviceWorker' in navigator) {
	navigator.serviceWorker.register('<?php echo $setting['domain']; ?>/service_worker.js')
	  .then((registration) => {
...
<script data-cfasync="false">
	...
	var logMode = <?php echo $setting['log_mode']; ?>;
	var leftMode = <?php echo $setting['left_mode']; ?>;
	var systemLoaded = 0;
</script>
<?php } ?>
</head>
